...
DEFAULT_CONFIG = {
    ...
    "display": {
        ...
        # Нижний ряд каждого столбика (обычно заказы этого маркетплейса) —
        # заменить на ОБЩУЮ выручку по всем маркетплейсам сразу (то же
        # число, что в самом верху обычного экрана) — одно и то же значение
        # повторяется под каждым столбиком, для наглядного "доля каждого от
        # общего" рядом с его собственной выручкой сверху.
