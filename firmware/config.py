...
DEFAULT_CONFIG = {
    ...
    # show_fbs_test_label — тестовый чекбокс в вебе: принудительно рисует
    # текст "Собрать FBS" на экране (см. fbs_label.* в assets/layout*.txt),
    # чтобы можно было проверить расположение/шрифт до того, как появится
    # реальная логика "есть несобранные FBS-заказы".
    "display": {
        "driver": "epd1in54",
        "screen": "200x200",
        "rotation": 0,
    ...
    "buzzer": {
        ...
        # Отдельный переключатель конкретно для сигнала при включении/
        # перезагрузке платы (main.py: beep() + startup_chime() сразу после
        # подключения к Wi-Fi) — независим от "enabled" выше (тот глушит
        # вообще всё; этот — только звук загрузки, продажи/тест по-прежнему
        # звучат). Проверяется ОДИН раз, в main.py при старте — в отличие
        # от enabled/volume/volume_curve, тут нет живого объекта Buzzer,
        # который можно поменять на лету, только со следующей перезагрузки.
        "boot_sound": True,
    },
