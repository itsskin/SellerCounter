...
# 10 минут, не 30с — raw-REPL сессии при заливке кода по USB (см.
...
# реально идёт полезная работа — HW-подтверждено: заливка даже одного
# файла в несколько сотен КБ (шрифт) стабильно роняла плату по watchdog при
# 30с таймауте. Пользователь не заметит разницы: 10 минут до автовосстановления
# после НАСТОЯЩЕГО зависания (баг, а не заливка) — не критично для
# витринного счётчика продаж, в отличие от заливки прошивки, которая
# происходит регулярно и должна быть надёжной.
WDT_TIMEOUT_MS = 10 * 60_000
