...
# 1 минута, не 30с — raw-REPL сессии при заливке кода по USB (см.
# tools/mpy-raw-serial) полностью останавливают event loop (а значит и
# кормёжку) на всё время, пока плата в raw REPL, независимо от того, сколько
# реально идёт полезная работа — HW-подтверждено: заливка даже одного файла
# в несколько сотен КБ стабильно роняла плату по watchdog при 30с таймауте.
# 10 минут держали раньше, пока в OTA-манифесте были все размеры шрифтов
# (~2.3МБ) — после того как набор шрифтов урезали до реально используемых
# (~600КБ, см. generate_verdana_font.py/generate_orbitron_font.py), такой
# большой запас больше не нужен — 1 минута с головой хватает и на заливку
# одного файла по USB, и на OTA-обновление по Wi-Fi, при этом
# автовосстановление после НАСТОЯЩЕГО зависания остаётся быстрым.
WDT_TIMEOUT_MS = 60_000
