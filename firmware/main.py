...
def main():
    ...
    # общего "выключить звук" — тот глушит вообще всё, эти только звук
    # загрузки):
    #  - PWRON_RESET (физическое включение/подключение питания) — обычная
    #    мелодия "всё готово" (buzzer.boot_sound), как и раньше.
    #  - WDT_RESET (самостоятельный сброс по watchdog, см. _feed_watchdog
    #    ниже) — ДРУГОЙ, специально узнаваемый на слух сигнал (чистый тон
    #    440Гц/3с, "нота Ля") вместо обычной мелодии (buzzer.watchdog_sound).
    #    По просьбе пользователя — чтобы на слух сразу отличать
    #    самостоятельный watchdog-краш от обычной перезагрузки, не
    #    заглядывая каждый раз в лог через serial.
    #  - Всё остальное (обычный machine.reset() — сохранение настроек, OTA,
    #    заливка кода и т.п.) — молчим. Раньше играла та же мелодия, что и
    #    на PWRON, но таких перезагрузок в обычной жизни платы (настройки,
    #    OTA) достаточно много, чтобы звук на каждую из них был просто
    #    шумом, а не полезным сигналом.
    ...
    if cause == machine.PWRON_RESET:
        ...
        if cfg["buzzer"].get("watchdog_sound", True):
            buzzer.beep(440, 3000)
